...
<div class="page">
  ...
  <div class="quick-actions">
    <button class="action-card exercise" on:click={() => navigate('log-exercise')} type="button">
      <span class="action-icon">🏋️</span>
      <span class="action-label">Log Exercise</span>
    </button>
    <button class="action-card pain" on:click={() => navigate('log-pain')} type="button">
...
<style>
  ...
  .date-label { font-size: 0.875rem; color: var(--color-text-muted); margin-top: 0.2rem; }
  ...
  .action-label { font-weight: 700; font-size: 0.875rem; color: var(--color-text); }

  h2 { margin-bottom: 0.75rem; }
  .empty-today { color: var(--color-text-muted); font-size: 0.9rem; }
  .summary-chips { display: flex; gap: 0.5rem; flex-wrap: wrap; }

  .chip { font-size: 0.8rem; font-weight: 600; padding: 0.35rem 0.85rem; border-radius: 2rem; }
